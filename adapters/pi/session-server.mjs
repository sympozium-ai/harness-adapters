...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    const response = await (queue = queue.catch(() => undefined).then(() => runPi(prompt, sessionID, stream ? (chunk) => writeSSE(res, { id, object: "chat.completion.chunk", created: Math.floor(Date.now() / 1000), model: process.env.MODEL_NAME, choices: [{ index: 0, delta: { content: chunk }, finish_reason: null }] }) : undefined)));
